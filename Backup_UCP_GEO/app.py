            ...
            if fast_mode:
                with st.spinner("Executing Fast Scan..."):
                    ...
                    if response.status_code == 200:
                        data = extract(response.text, base_url=url, uniform=True, syntaxes=['json-ld'])
                        ...
                        with u_col2:
                            ...
                            for u_key, u_item in audit_data['results'].items():
                                ...
                                if u_item['status'] == 'Green':
                                    st.success(f"✅ {u_label} (Passed) {score_str}")
                                ...
                                if u_item['status'] != 'Green':
                                    with st.expander(f"How to fix {u_item['name']}?", expanded=False):
                                        if u_key == "gtin":
                                            ...
                                            st.markdown("""
                                            **Connect your brand and images to your product.** AI agents require structured identity data 
                                            and verified image URLs to summarize your product accurately for users. 

                                            Add a `brand` object and an `image` array **directly within your Product schema**:
                                            ```json
                                            {
                                                "@type": "Product",
                                                "brand": {
                                                ...
                                                "image": [
                                                    "https://yourstore.com/images/product-main.jpg",
                                                    "https://yourstore.com/images/product-angle.jpg"
                                                ]
                                            }
                                            ```
                                            """)
